 ...
 function initMap() {
     ...
     $.each(jsonlocations, function(i, obj) {
         ...
         var editScoreValue = obj.score;
         if (type == 'f') {
             checkfinal = 'yes';
         }
         latlng = new google.maps.LatLng(lat, lon);
         placeMarker(latlng, map, type, control_id, editScoreValue);
     });
 ...
 placeMarker = function(location, map, type, controlid, scoreVal) {
     ...
     if (checkControl == 'startButton' || type == 's') {
         ...
         submitScore(controlid, type, scoreVal);
     } else {
         $('#myModal').modal('show');
     }
 ...
 function submitScore(controlId, typec, score) {
     ...
     if (exist == 1) {
         ...
         if (checkControl == 'controlButton' || typec == 'c') {
 ...
 function removeScore(controlId) {
     $.each(locations, function(index, val) {
         if (val != undefined) {
             if (val.controlid == controlId) {
                 locations.splice(index, 1)
